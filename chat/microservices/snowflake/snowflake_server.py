...
from factory import serve, get_or_flick_instance_id
...
async def entrypoint(starter: AsyncMultiProcessManager):
    ...
    async with move_on_starter(starter):
        instance_id = await get_or_flick_instance_id(starter)
        shared_array = starter.process_manager.shared_array
        # it's simple now, worker checks the mask then decrease one
        shared_array[0] = starter.process_manager.num_workers | 0x10  # mask
        shared_array[1] = instance_id
